...
exports.list = async (req, res) => {
    try {
      const { _offset, _limit, _sort, _order, q, _time,_group, ...rest } = req.query;
      const query = { raw: true, include: [ {model : Order, attributes: ["createdAt","id"]}], order: [['createdAt','DESC']] };
      ...
      if(_time){
        ...
        const formattedNextMonthDate = nextMonthDate.format("YYYY-MM-DD HH:mm:ss")
        query.where = {...query.where, createdAt:{[Op.between]: [forrmatedCurrentDate,formattedNextMonthDate]}}
      }
...
exports.updateReview = async (req, res) => {
    try {
      ...
      if(response[0] == 0) res.status(404).json("Cập nhật thất bại, id không tìm thấy")
