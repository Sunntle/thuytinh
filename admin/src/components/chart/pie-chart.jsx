...
const PieChart = (props) => {
  ...
  const calculatePercentage = (name_category) => {
    ...
    const percentages = props?.data.reduce((result, item) => {
      result[item.name_category] = ((item.productCount / total) * 100).toFixed(
        2
      );
      return result;
    }, {});
    ...
    <>
      ...
      <div className="flex flex-col gap-4">
        {props?.data.map((item, i) => (
          <div
            className="flex justify-center items-center w-full gap-x-5"
            key={i}
          >
            <Avatar
              shape="square"
              src={item.thumbnail}
              className="w-1.5/12 bg-[#fde3cf] text-[#f56a00]"
            />
            <span className="w-8/12 font-medium">
              {item.name_category} ({calculatePercentage(item.name_category)}%){" "}
            </span>
